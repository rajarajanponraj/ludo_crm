    ...
    app.component('v-datagrid-filter', {
        ...
        methods: {
            ...
            createOrUpdateFilter(params, { setErrors }) {
                ...
                this.$axios.post(params.id ? `{{ route('admin.datagrid.saved_filters.update', 'place_holder_id') }}`.replace('place_holder_id', params.id) : "{{ route('admin.datagrid.saved_filters.store') }}", {
                    src: this.src,
                    applied,
                    ...params,
                })
            ...
            deleteSavedFilter(filter) {
                this.$emitter.emit('open-confirm-modal', {
                    agree: () => {
                        this.$axios.delete(`{{ route('admin.datagrid.saved_filters.destroy', 'place_holder_id') }}`.replace('place_holder_id', filter.id))
                            .then(response => {
