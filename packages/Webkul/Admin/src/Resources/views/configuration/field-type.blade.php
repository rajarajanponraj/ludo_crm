...
<div class="mb-4 last:!mb-0">
    <v-configurable name="{{ $field->getNameField() }}" value="{{ $value }}" label="{{ trans($field->getTitle()) }}"
        ...
        src="{{ Storage::url($value) }}" field-data="{{ json_encode($field) }}"
...
@pushOnce('scripts')
    <script type="text/x-template" id="v-configurable-template">
            <x-admin::form.control-group class="last:!mb-0">
                ...
                <template v-if="field.type == 'file' && field.is_visible">
                    <a
                        v-if="value"
                        :href="`{{ route('admin.configuration.download', [request()->route('slug'), request()->route('slug2'), 'place_holder_id']) }}`.replace('place_holder_id', value.split('/')[1])"
                    >
